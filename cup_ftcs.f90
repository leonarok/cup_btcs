...
module declarations
	...
	double precision,public :: Ra,Pr_a,beta_a,k_a,nu_a,h_co
...
!	            	(y(j+1)-y_v(j+1))+k(i,j+1)*(y_v(j+1)-y(j))))*arean
	        	...
	        	if (i==2) then
	        		...
	        		h_co=k_a/areaw*( 0.68+0.67*Ra**0.25/	&
	        			( 1+(0.492/Pr_a)**(9/16) )**(4/9) )    		
	        		h_tot=1/(cup_width/k_cup + 1/h_co)
	        		
	          		su=su-h_tot*areaw*(T(i,j)-T_amb)
	        	elseif (i==npi-1) then
	        		Ra=g_const/(nu_a*beta_a*T(i,j))*(T(i,j)-T_amb)*areae**3
	        		h_co=k_a/areae*( 0.68+0.67*Ra**0.25/	&
	        			( 1+(0.492/Pr_a)**(9/16) )**(4/9) )
	        		h_tot=1/(cup_width/k_cup + 1/h_co)
	        	...
	        	if (j==2) then
	        		Ra=g_const/(nu_a*beta_a*T(i,j))*(T(i,j)-T_amb)*areaw**3
	        		h_co=k_a*0.54*Ra**0.25/areaw
	        		h_tot=1/(cup_width/k_cup + 1/h_co)
	        		
	          		su=su-h_tot*areas*(T(i,j)-T_amb)
	        	elseif (j==npj-1) then
	        		Ra=g_const/(nu_a*beta_a*T(i,j))*(T(i,j)-T_amb)*areaw**3
	        		h_co=k_a*0.27*Ra**0.25/areaw
	        		h_tot=1/(cup_width/k_cup + 1/h_co)
